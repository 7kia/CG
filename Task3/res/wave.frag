...
void main()
{
    ...
    vec4 color = texture2D(waveTexture, fragTextureUV.st);

    vec4 diffuseIntensity = vec4(factors.diffuse) * color * light0.diffuse;

    vec4 specularIntensity = color * factors.specular * light0.specular;
