...
void CArchimedeanSpiral::Redraw() const
{
	...
	{
		glVertex2f(m_origin.x + (GetX(angle) * SCALE_FACTOR), m_origin.y + (GetFunctionValue(angle) * SCALE_FACTOR));
	}
...
float CArchimedeanSpiral::GetFunctionValue(float angle) const
{
